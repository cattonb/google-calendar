  ...
  availabilities: z
    .array(
      z.object({
        ...
        startTime: z
          .string()
          .regex(/^([0-9]|0[0-9]|1[0-9]|2[0-3]):[0-5][0-9]$/, "Time mist be in the format HH:MM"),
        endTime: z
          .string()
          .regex(/^([0-9]|0[0-9]|1[0-9]|2[0-3]):[0-5][0-9]$/, "Time mist be in the format HH:MM"),
      })
    )
    .superRefine((availablities, ctx) => {
      availablities.forEach((availability, index) => {
        const overlaps = availablities.some((a, i) => {
          return (
            i !== index &&
