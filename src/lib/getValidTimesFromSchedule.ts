import { AvailabilityWithIndex, GroupedAvailabilities } from "@/components/forms/ScheduleForm";
...
export async function getValidTimesFromSchedule(
  ...
  const schedule = await db.query.ScheduleTable.findFirst({
    where: ({ clerkUserId: userId }, { eq }) => eq(userId, event.clerkUserId),
    with: { availabilities: true },
  });

  if (schedule == null) return [];

  const groupedAvailabilities = schedule.availabilities
  ...
  return timesInOrder.filter((intervalDate) => {
    const availabilites = getAvailabilities(groupedAvailabilities, intervalDate, schedule.timezone);
    ...
    return eventTimes.every((eventTime) => {
      return (
        !areIntervalsOverlapping(eventTime as { start: Date; end: Date }, eventInterval) &&
        availabilites.some((availability) => {
          return (
